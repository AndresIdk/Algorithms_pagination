...
class LruFrame(ttk.Frame):
    def __init__(self, parent):
        ...
        self.tree = ttk.Treeview(
            self.frame, style="EstiloTabla.Treeview", columns=('#0', 'Referencia')
        )
    ...
    def lru_put(self, key, value):
        if key in self.lru_cache:
            self.actual.remove(key)
        elif len(self.lru_cache) >= self.memoria:
            least_recently_used = self.actual[0]
            del self.lru_cache[least_recently_used]
